...
const NavDropdown = (props: NavdropdownProps) => {
  return (
    <div className="lg:hidden absolute top-12 right-0 z-10 w-48 py-2 mt-2 text-sm text-gray-700 border border-none rounded-lg shadow-xl cursor-pointer bg-[#2e6e53]">
      {props.isAuthenticated ?
        <div>
          ...
          <div className='flex hover:bg-gray-200 items-center w-full justify-center relativ p-4  h-[50px] bg-white justify-self-end relative z-20'>
            <button onClick={() => props.logout()} className="flex items-center w-full justify-center relativ p-4  h-[50px] justify-self-end relative z-20">
              <p>Logg ut</p>
              <LogOutIcon className="w-4 h-4 m-3" />
            </button>
          </div>

          <div className="text-white text-[20px] p-3  md:ml-4 rounded-[10px] hover:bg-green-800 cursor-pointer">Kvittering</div>
          <div className="text-white  text-[20px] p-3 md:ml-4 rounded-[10px] hover:bg-green-800 cursor-pointer" >Søknad</div>
          <div className="text-white  text-[20px] p-3 md:ml-4 rounded-[10px] hover:bg-green-800  cursor-pointer">Min side</div>
        </div>
        :

        <div className='flex hover:bg-gray-200 items-center w-full justify-center relativ p-4  h-[50px] bg-white justify-self-end relative z-20'>
          <button onClick={() => props.login()} className="flex items-center w-full justify-center relativ p-4  h-[50px] justify-self-end relative z-20">
  ...
  return (
    <div className="bg-[#2e6e53] flex">
      <div className="flex items-center justify-between p-3 w-full">
        ...
        <button onClick={toggleNavbarDropdown} className="flex justify-end abolute right-0 lg:hidden">
          <Bars3Icon
            className={`cursor-pointer text-white h-9 relative justify-self-end absolute top-0 right-0 transition-transform transform ${showNavDropdown ? "rotate-45 opacity-0" : "rotate-0 opacity-100"
              }`}
          />
          <XMarkIcon
            className={`cursor-pointer text-white h-9 relative justify-self-end absolute top-0 right-0 transition-transform transform ${showNavDropdown ? "rotate-0 opacity-100" : "rotate-45 opacity-0 hidden"
              }`}
          />
        </button>
        {showNavDropdown &&
          <NavDropdown isAuthenticated={isAuthenticated} user={user} logout={logout} login={loginWithRedirect} />
        }
